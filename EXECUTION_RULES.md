...
> **This file is the contract.** If you are an AI coding agent working on this project, you MUST follow every rule in this file. If any rule conflicts with plan.md, this file wins. If you cannot follow a rule, STOP and report why.
...
- Pick exactly ONE item from `plan.md` (e.g., "T0-1").
...
```
Step 1: Read this file (EXECUTION_RULES.md) completely.
Step 2: Read plan.md completely.
Step 3: Find the LOWEST-NUMBERED ticket that is not yet done.
        - Tier 0 tickets first (T0-1, T0-2, ... T0-13).
        - Then Tier 1 (T1-1, T1-2, ...).
        - Then Tier 2, Tier 3, Tier 4.
Step 4: Read the ticket's File(s), Bug, and Fix sections.
Step 5: Read the actual file(s) referenced in the ticket.
Step 6: Make the EXACT change described. No more, no less.
Step 7: Run `npm run build`. If it fails, fix or revert.
Step 8: Run `npm test`. If it fails, fix or revert.
Step 9: Commit: `git add -A && git commit -m "fix: T0-X short description"`
Step 10: Report what you did, what you verified, and any concerns.
Step 11: STOP. Wait for the user to say "continue" before starting the next ticket.
```
