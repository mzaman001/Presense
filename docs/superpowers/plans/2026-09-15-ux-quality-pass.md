...
- [ ] **New finding (2026-09-16), fold into the same rule**: `globals.css:17` resets Tailwind's entire default color palette (`--color-*: initial`) and never redefines `black`/`white`, so any `bg-black`/`text-white`/`fill-black`/`border-white`/etc. utility class silently compiles to nothing — a real, invisible-at-a-glance bug, not just an inconsistency. Confirmed 19 such occurrences across 11 files (`grep -rEo "(bg|text|fill|border|stroke|ring)-(black|white)\b" src/` for the current list) — one already caused a real user-visible bug (Task 1.1-adjacent: the Focus screen's play/pause button icon was fully invisible, fixed directly as `dc4197e`, not part of this Phase 0.5 task). The lint rule above should also catch bare `black`/`white` Tailwind utility names, not just literal hex/rgba, since they're equally dead in this codebase. Audit and fix the remaining 18 as part of this task.
