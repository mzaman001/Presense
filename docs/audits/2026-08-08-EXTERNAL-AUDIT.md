...
- **✅ DONE Aug 12, 2026:** `permissions: contents: read` added to `ci.yml` (only workflow missing it); `checkout` → `11d5960a…`, `setup-node` → `49933ea5…` SHA-pinned in `ci.yml` + `semgrep.yml` (SHAs resolved from the official `v4` tags via GitHub API); `rg "@v[0-9]" .github/workflows` → 0 hits. Dependabot `github-actions` not added — decision recorded: solo repo, manual pin refresh at upgrade time; revisit if the workflow set grows.
