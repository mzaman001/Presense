...
**Status — ✅ CLOSED Aug 12, 2026:** `ci.yml` now declares `permissions: contents: read` (no elevation needed — the lint/typecheck/test/build steps only read contents; the bundle-budget step needs nothing more); `actions/checkout` pinned to `11d5960a326750d5838078e36cf38b85af677262` and `actions/setup-node` to `49933ea5288caeca8642d1e84afbd3f7d6820020` (SHAs resolved from the official `v4` tags via the GitHub API at close-out time) in `ci.yml` and `semgrep.yml`; `osv-scanner.yml` was already SHA-pinned with a `permissions` block. `rg "(@v[0-9]|@v[0-9]+\.[0-9]+)" .github/workflows` → 0 hits. Dependabot `github-actions` updates not added — recorded decision: solo repo, pins refreshed manually at upgrade time; revisit if the workflow set grows.
