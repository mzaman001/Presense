...
**Status — ✅ CLOSED Aug 17, 2026 (commit `[33m7ced9bf[m`).** All five acceptance criteria are met. **AC1/AC2** were satisfied by INFRA-19, which funnels every status write on the five entity tables through typed patches in `src/lib/item-lifecycle.ts` — a repo-wide grep confirms zero direct `status:` literals outside that module; the canonical deletion patch is `moveItemToTrashPatch()` (`status: "deleted"` + `deleted_at`), while the completed-tasks archive keeps its distinct `status: "archived"` semantics, and the two contradictory delete paths in `think/[id]/page.tsx` were already reconciled to the single soft-delete behavior. **AC3:** the existing `src/app/(app)/trash/page.tsx` already queries all five tables and offers restore + permanent delete; it now additionally honors `?filter=<item|thread|explore|person|location>` per the recorded **CONF-10 Option C** decision, and thin "check the trash" pointers with per-space filter links were added to the empty states of `/do`, `/think`, `/explore`, `/remember/people`, and `/remember/locations` — one implementation, no duplicated trash logic. **AC4:** `supabase/functions/cron_cleanup` hard-deletes all five tables at a 30-day retention window (existing mechanism, verified as-is). **AC5:** people and locations delete via the same soft-delete patch and are fully restorable from `/trash` (list queries on both pages additionally filter `status != deleted` so trashed rows never render on any client). `npm test` 181/181, `VERCEL=1 npm run build` exit 0.
...
**`INFRA-09` status — ✅ CLOSED Aug 17, 2026 (audit-verified, no code change; commit `[33m4d2e1ca[m`).** Audit of `supabase/functions/cron_cleanup/index.ts` confirms the retention job already exists and meets the AC in full: it permanently deletes rows past the 30-day window on all five entity tables (`items`, `threads`, `explores`, `people`, `locations`) with the correct compound filter (`status = 'deleted' AND deleted_at IS NOT NULL AND deleted_at <= now − 30d`), runs the five deletes in parallel, and explicitly checks `res.error` on every result before returning — nothing was found to extend, and duplicating it elsewhere would violate its own "audited/extended, not duplicated" directive. Three operational items recorded here (the ticket is docs/verification, the schedule itself lives outside the repo, consistent with the `SEC2-02` decision record): **(a)** the invocation trigger is the Supabase Dashboard scheduled-functions configuration (no `pg_cron`/`net.http_post` exists in the repo — by design, per the `SEC2-02` status record, which also carries the defense-in-depth 401 guard both functions now have); **(b)** launch action already recorded in `SEC2-02`: confirm the Dashboard schedule sends an `Authorization` header with a JWT (anon key sufficient — the function holds its own service-role key), since a missing header means the purge silently never runs; one-time verification is invoking the function manually from the Dashboard with the auth header and confirming `200` plus expected side effects within 24h of logs; **(c)** the smoke check is wired to `OBS-01`'s closed Sentry adoption: a daily "did the cron run" check on the scheduled invocations (zero invocations or a 401 spike is the failure signal — a follow-up candidate if no Sentry function-monitor is configured in production). **Note:** the function must be deployed to the hosted project (`supabase functions deploy cron_cleanup`) if it has not been deployed since the `SEC2-02` 401-guard change; the sandbox cannot deploy (no Supabase CLI access), so deployment is a launch-action step, not a code step. This closure also completes `BUG-08` AC4 as "audit-verified as-is."
...
**Status — ✅ CLOSED Aug 17, 2026 (commit `[33me96b692[m`).** A merged, enforceable contract exists at `docs/project/INTERACTION_PATTERNS.md`, stating exactly one allowed pattern each for the five required areas, each with a single canonical code example verified against the current build: (1) **creation entry points** — per-space structured creators (TaskAddPanel, thread composer, ExploreDrawer, add-person/add-location panels) opened identically by header "+" and empty-state "Add" actions (`BUG-04`/`BUG-24` verified: all empty-state buttons match their headers; Inbox's Quick Capture routing is the single documented exception); (2) **editing** — the same dual-mode sheet/panel handles create and edit, distinguished only by the edit handle, never a second screen or an NLP re-parse; (3) **delete/undo/confirm** — the three-stage model: `moveItemToTrashPatch()` soft delete + 5-second Undo toast → restore from `/trash` (global surface, `?filter=` type param, `CONF-10` Option C) → `ConfirmModal` destructive confirmation for permanent delete; archive (`status: "archived"` = done) remains distinct from trash (`BUG-08`/`CONF-04`); (4) **toast conventions** — sonner success toasts with Undo actions on reversible optimistic writes, explicit error toasts on failure, `safeMutate()` for all client mutations; (5) **keyboard shortcuts** — one global handler in `AppContentWrapper.tsx` (`⌘K`/`c`/`/`/`1`–`6`/`Escape`), no component-level global listeners. The contract references `docs/architecture/status-lifecycle-vocabulary.md` for status meanings. **Repo-reference note:** the ticket's acceptance criterion calls for referencing the doc from `CLAUDE.md`'s "key files to read" list; `CLAUDE.md` lives in the user's Antigravity config (`C:\Users\muhdz\.gemini\antigravity`), not the repo, and is not reachable from the sandbox — the repo-side pointer is instead recorded in `AGENTS.md` §2's file-location table (the single entry point all coding agents read). A line should be added to the Antigravity key-files list when the user's agent config is next edited. `AGENTS.md` is the only file touched besides the new doc; no code changed. Build and tests unaffected (doc-only PR).
...
- **✅ RESOLVED — Aug 17, 2026 (user decision): Option C — global `/trash` as the primary surface, with thin per-space pointers.** Rationale recorded: (1) **recovery is the product promise** — Presense's calm/trustworthy mandate leans on "nothing you delete is lost for 30 days, and you can always find it"; a single predictable destination serves that, and trash recovery is a rare, high-anxiety event where the user may not remember which space held the item months later; (2) **zero duplicated implementation** — one shared `/trash` with type filters (`?filter=thread` etc., building on the existing `explore/trash/page.tsx` queries) gives per-space views for free, while five per-space copies would reopen exactly the duplication channel through which BUG-08's original drift occurred; (3) **the v5 plan's objection is satisfied** — "over-engineered" applies to complex multi-tenant permission-scoped trashes; Presense is single-user and the global trash is trivially bounded, and per-space "N items in trash → `/trash?filter=x`" pointers on empty/error states cost one line of UI each without duplicating any trash logic. The v5 plan's valid points retained as design constraints for BUG-08: the global trash stays visually light (one component, simple list + type chips), delete actions keep per-space undo affordances (interaction-level, DS-11's scope — not trash-surface duplication), and no new nav weight beyond the single `/trash` route. `BUG-08` acceptance criterion 3 stays as written (single global surface, generalized from `explore/trash`); BUG-08 is now unblocked; `DS-11` and `INFRA-09` (retention purge) unblocked in turn.
...
**Status — ✅ CLOSED Aug 17, 2026 (commit `[33m131bbaf[m`).** Delivered as a single new migration, `supabase/migrations/20260817000000_rls_cache_auth_uid.sql` — no already-applied migration was edited in place (invariant 6) and no `auth.uid() = user_id` clause was removed (stop-list item 5): every `users_own_*` policy was drop-and-recreated with `(select auth.uid()) = user_id`, identical permitted row set. Audit of all 26 migration files found exactly 14 bare `auth.uid()` occurrences inside `USING`/`WITH CHECK`, concentrated in three files: `001_baseline.sql` (9 policies), `20260628121249_ritual_tracking.sql` (original ritual-logs policy), and `20260703000001_rls_to_authenticated.sql` — the authoritative live state, a `DO $$` loop that re-creates `users_own_*` for `items, people, threads, explores, locations, push_subscriptions, user_settings, categories, session_logs, ritual_logs` plus a second block for the legacy policy name. The new migration mirrors that exact loop structure for idempotency. Two other files contain `auth.uid()` (both `rename_category` RPCs: `009_rename_category_rpc.sql`, `20260703000003_harden_rename_category.sql`) but as PL/pgSQL `v_user_id := auth.uid()` assignment-once patterns, not per-row policy expressions — out of scope per the ticket, left untouched. The EXPLAIN ANALYZE acceptance requires a live Supabase connection (no CLI available in this sandbox), so mechanical evidence is recorded instead: `grep` confirms the new migration contains the cached form in every policy expression, `git status`/`git diff` confirm zero changes to prior migration files, build green (VERCEL=1), tests 181/181.
...
**Status — ✅ CLOSED Aug 17, 2026 (commit `[33md43d490[m`).** Delivered as a single new migration, `supabase/migrations/20260817000001_rls_split_for_all.sql` — no already-applied migration was edited (invariant 6). The live state (post-INFRA-15 `20260817000000`) is ten `users_own_*` policies, one `FOR ALL` per table (`items, people, threads, explores, locations, push_subscriptions, user_settings, categories, session_logs, ritual_logs`), all in the cached `(select auth.uid()) = user_id` form; the legacy `Users can manage their own ritual logs` policy name from `20260628121249` is also handled. The migration clears any policy in each `users_own_*` family (the old FOR ALL or any `_select/_insert/_update/_delete` variant from a partial run, plus the legacy name) in an inner loop, then creates four operation-scoped policies per table: `SELECT` with `USING`, `INSERT` with `WITH CHECK`, `UPDATE` with both `USING` and `WITH CHECK`, `DELETE` with `USING` only — every expression preserves the INFRA-15 cached form, so the permitted row set is identical and only the planner's per-operation path changes. Acceptance verified against a real PostgreSQL 16 instance (not assumed): with all 10 tables present and both live-state scenarios seeded, both a first run and a re-run (idempotency) complete without error, yielding exactly 40 policies (10 each of SELECT/INSERT/UPDATE/DELETE), zero `FOR ALL`, zero legacy names, and every table's `UPDATE` policy paired with a `SELECT` policy — the documented gotcha holds by construction. Build green (VERCEL=1), tests 181/181.
...
- **Status — ✅ CLOSED Aug 17, 2026 (commit `[33m7715e13[m`).** Every status write on an entity table now goes through `src/lib/item-lifecycle.ts`; a repo-wide grep for direct `status:` literals in `src/` and `supabase/functions/` returns zero results outside the module and its tests. New patches added: `completeTaskPatch` (done + completed_at), `uncompleteTaskPatch`, `activateItemPatch` (inbox→Do), `activateItemWithDeadlinePatch` (triage), `revertItemPatch` (triage undo), `archiveThreadPatch`, `restoreItemPatch`, `moveItemToTrashPatch`, `newTaskInsert`, `permanentlyDeleteFilter`. Two latent bugs fixed in passing: Home dismiss wrote `status: "deleted"` without `deleted_at` (unrecoverable trash row), and done/uncomplete could be written without the matching `completed_at` pair. Settled vocabulary documented at `docs/architecture/status-lifecycle-vocabulary.md`. Verified with `tsc --noEmit` (clean), `VERCEL=1 npm run build` (exit 0), and `npm test` (181/181).
...
| `BUG-05` / `CONF-08` calendar | **DONE** — verified Aug 16, 2026: all four root causes are closed (`c91da44` grid restructure fixed the header/grid desync and removed the NLP capture round-trip; `50c6f03` removed the `calc(100dvh-180px)` magic height in favor of flex-constrained `h-full` parents and added the mobile day-view default; `df60f12` added keyboard navigation to slots, chips, and both views). Final residual keyboard gap in MonthView's "+N more" popover (Escape close, ARIA) closed in `06ec42c`. Incremental repair path was chosen and completed — `T3-1` full rewrite is no longer required; all ticket acceptance criteria met. | |
...
| `PERF-03` ambient orb mobile cap / visibility pause | **DONE** (Aug 17, 2026) | Code fix `[33mac01a48[m`: `AmbientBackground.tsx` now pauses all four orb animations whenever the tab is hidden (`visibilitychange` → `paused` state, same discipline as PERF-18's `after()` and RitualOverlay's listener), and caps the visible orb count at two (`orb-1`/`orb-2`) on screens matching `(max-width: 767px)` via `useMediaQuery` — the cap applies only when ambient bg is enabled and the tab is visible, survives viewport resize, and keeps the noise layer always mounted. Orbs 3/4 are clipped off-screen on mobile anyway, so the cap removes compositor work with no visible change. Build green (VERCEL=1), tests 181/181. |
| `PERF-04`/`PERF-08` backdrop-filter reduction | **DONE** — verified Aug 16, 2026: blur values reduced (modal 48→24px, elevated 32→20px, dropdown 20→18px; hierarchy preserved), `contain: paint` on every blur-bearing glass primitive, reduced-transparency fallback verified complete (mobile chrome bars added to override list). Commit `b8c25b8`. | |
...
| `PERF-04`/`PERF-08` backdrop-filter budget | **DONE, verified Aug 16, 2026** | `globals.css` (commit `b8c25b8`): the heaviest blur values were reduced to cut compositing cost — `--blur-modal` 48→24px, `--blur-elevated` 32→20px, `--blur-dropdown` 20→18px, with the hierarchy strictly preserved (modal 24 > elevated 20 > dropdown 18 > card 16 > chrome 12); `contain: paint` added to every blur-bearing primitive (`.glass-panel`, `.glass-card`, `.glass-card-elevated`, `.card-upcoming`, `.sidebar`, `.modal`, `.dropdown-panel`, `.mobile-top-bar`, `.bottom-nav`, `.mobile-drawer`, `[data-sonner-toast]` — both dark and light variants), so repaints inside a blur surface no longer invalidate siblings; and the `@media (prefers-reduced-transparency: reduce)` fallback was verified complete — all five blur tokens forced `none`, surfaces blended opaque, and the three mobile chrome bars (previously missing from the override list) were added. The raw occurrence count (~23) was intentionally not reduced — that consolidation is `DS-04`'s design work, not this ticket's scope — but each occurrence is now cheaper (containment + lower radii + working fallback). |
...
## 29. Addendum 16 — external audit Aug 8, 2026: triage of `docs/audits/2026-08-08-EXTERNAL-AUDIT.md` (Aug 10, 2026)

Source: `docs/audits/2026-08-08-EXTERNAL-AUDIT.md` (Claude, acting as external reviewer; 36 KB; every finding file:line-verified on Aug 8, 2026). Triage performed Aug 10, 2026 after the agent re-verified a sample of the claims against the code (all accurate) and confirmed no finding appears anywhere in the canonical backlog.

**Verdict on the audit's own correctness (spot-checked Aug 10, 2026):** SEC2-01 (`rate-limit.ts:29-34` hardcoded singleton) — accurate; CI-01 (`.github/workflows/eslint.yml` installs eslint@8.10.0 + `--config .eslintrc.js` + `continue-on-error: true`; verified `.eslintrc.js`/`.eslintrc.json` do not exist) — accurate; PWA2-01 (`manifest.json:13-14` reuses any-purpose icons as maskable) — accurate. OBS-01 is corroborated by `CONTEXT.md`'s own `/api/telemetry` "black hole" note (that note is now struck through — OBS-01 CLOSED Aug 12, 2026, commit `83a95e1`).

**ID mapping (provisional → final):**

| Audit §1 ID (provisional) | Verdict | Final ticket |
|---|---|---|
| `SEC2-01` | new | `SEC2-01` |
| `OBS-01` | new | `OBS-01` |
| `CI-01` | new | `CI-01` |
...
| `CI-04` | new | `CI-04` |
| `PWA2-01` | new | `PWA2-01` |
| `PERF-07` | **duplicate — no new ticket** | already tracked as `INFRA-15` (Critical; identical requirement); the audit's added "verify a `user_id` index exists per table" is already `INFRA-17` |
| `INFRA-15` | renamed (ID collides with existing RLS ticket) | `INFRA-23` |
| `SEC2-02` | new | `SEC2-02` |
| `PERF-08` | renamed (ID collides with existing backdrop-filter ticket) | `PERF-13` |

**Stale claims in the audit (closed since Aug 8 — do not re-fix):** §7 ship-blocker 3 ("finish BUG-38") and 4 ("warm-light theme") were both closed Aug 10, 2026 (see §24.3/§24.7 and `DOCS_NEEDS_CODE.md`); §6's "BUG-38 remains unresolved" is likewise superseded (commit `660f5a3`). The audit's only remaining live dependency note is OBS-01's sequencing with BUG-38's "report to the tracker" contract — that contract now has a target in OBS-01.

### New tickets

**SEC2-01 — Parameterize the rate limiter so per-call-site limits actually apply [Critical].** Files: `src/lib/rate-limit.ts:26-34` (module-level singleton `Ratelimit` hardcoded to `slidingWindow(100, "60 s")` with `prefix: "rl:capture"`), `src/app/api/account/route.ts:19` (intends 3/min for destructive delete-account), `src/app/api/capture/route.ts:16`. The per-call `maxRequests`/`windowMs` arguments are silently ignored whenever Redis is configured — only the never-configured in-memory dev fallback honors them. Net effect: account deletion is rate-limited to 100/min and shares its counter with capture traffic. Requirement: parameterize `Ratelimit` construction (e.g. `Map<string, Ratelimit>` keyed by caller-supplied bucket name) so each call site's limit and prefix are respected; keep the in-memory fallback behavior. Acceptance: unit test asserting a 4th account-delete request within a minute is rejected (and that capture's own limits are unchanged); review passes on the Redis-backed path, not just the fallback.

**Status — ✅ CLOSED Aug 10, 2026 (commit `e895df8`):** `src/lib/rate-limit.ts` now uses a bucket registry (`Map<string, Ratelimit>`): each call site gets its own lazily-constructed limiter with `slidingWindow(maxRequests, windowMs)` (window formatted as a `Duration` string, e.g. `"60 s"` — v2's type rejects bare numbers) and `prefix: "rl:<bucket>"`; signature is now `checkRateLimit(bucket, key, maxRequests, windowMs)`. Call sites: `account` (3/60 s), `capture` (100/60 s), `people-reorder` (30/60 s); in-memory fallback counters are namespaced per bucket (`${bucket}:${key}`), fail-closed-in-production and single-dev-warning behaviors preserved. Verification: `rate-limit.test.ts` extended to 7 tests — 4th account request within a minute rejected; capture counter unaffected by account traffic; window expiry re-allows; fail-closed retained; Redis-backed path proven via constructor assertions (per-bucket `slidingWindow(3, "60 s")`, `prefix "rl:account"`, exactly one instance per bucket). Full suite 173/173, `npm run build` green, lint 0 errors on all 5 touched files.

**OBS-01 — Wire a real production error/performance sink [Critical for release].** Files: `src/app/api/telemetry/route.ts:26-33` (Zod-validates then `console.warn` + 204 — the data is discarded; ephemeral, unalertable serverless log stream), `src/lib/logger.ts:1-24` (Pino with no transport), `package.json` (no `@sentry/*`). Nothing ships data anywhere durable — `BUG-38`'s `safeMutate()` "report to the tracker" contract and `AGENTS.md` invariant #1's "report to the error tracker" have no sink; `README.md` already labels logging a stub. Requirement: adopt **Sentry** (`@sentry/nextjs`, per `TOOL-06`) or, if Vercel-hosted, **Log Drains + Speed Insights + Web Analytics**; forward `/api/telemetry` to the chosen sink instead of `console.warn`; coordinate logger transport with `TOOL-05`; add a CSP `report-uri`/`report-to` to `src/proxy.ts` (audit §5) once the sink exists. Acceptance: a thrown client error and a 500 from an API route both appear in the tracker; `/api/telemetry` no longer discards payloads; CSP violation reports land in the same sink. Priority: Critical for release — sequence before any fix whose failure reporting depends on it.

**Status — ✅ CLOSED Aug 12, 2026 (commit `83a95e1`):** adopted **Sentry** (`@sentry/nextjs@10.70.0`), satisfying `TOOL-06`; `TOOL-05` (Pino log-drain transport) deliberately unchanged with recorded decision — Sentry captures errors/context, a separate log-drain is TOOL-05's scope, evaluated against Sentry breadcrumbs per TOOL-05's no-two-overlapping-systems requirement. Client init lives in `src/instrumentation-client.ts` (Next 16 auto-loads it as client instrumentation — plan-mode's "dead code, delete" claim was **wrong**, corrected at execution; it was a live pipeline to a discarding route) plus `sentry.server.config.ts`/`sentry.edge.config.ts` per current SDK naming, all DSN-gated with the `.catch()` env pattern (invariant #1: no DSN = today's behavior, never throws); `next.config.ts` wrapped `withSentryConfig(analyze(withSerwist(...)))` — Turbopack build green twice, documented fallback not needed. `/api/telemetry` now forwards (`client-error` → `captureMessage` level "error", `web-vital` → level "info", context extras; Zod/400/204 contract unchanged); explicit `Sentry.captureException` added to the 3 try/catch API routes (`account`, `capture`, `people/reorder`; `auth/callback` has no catch — Supabase returns errors, doesn't throw). CSP `report-uri` derived from the DSN in `src/proxy.ts` (audit §5; EU `ingest.de.sentry.io` host preserved; byte-identical CSP when DSN absent). Verification: `telemetry-route.test.ts` 5 tests, `middleware.test.ts` 12 tests (EU/US DSN derivation, absent + present end-to-end), full suite 181/181 sequential, `npm run build` green, lint-staged 0 errors. Acceptance met: a thrown client error and an API 500 both reach Sentry, `/api/telemetry` no longer discards, CSP violations land in the same sink. Follow-ups recorded: source-map uploads (`SENTRY_AUTH_TOKEN` + `.sentryclirc`) and build-upload release pipeline; account route's handled `deleteUser` failure branch (returns 500 without throwing) is a candidate capture site; Sentry Replay/performance sampling tuning after first production week.

**CI-01 — Delete `eslint.yml`; it has never linted anything [High].** File: `.github/workflows/eslint.yml:26-40` — installs `eslint@8.10.0` fresh each run, lints with `--config .eslintrc.js` (file does not exist; verified Aug 10, 2026 — the repo is ESLint 9 + flat `eslint.config.mjs`), wrapped in `continue-on-error: true`, so the job is always green and shows a PR checkmark implying signal. Requirement: delete the workflow — `ci.yml`'s `npm run lint` is the real gate; if SARIF upload is wanted, add `--format @microsoft/eslint-formatter-sarif` to the existing `ci.yml` step against the real config. Acceptance: workflow file deleted; no `.eslintrc*` reference remains anywhere.

**Status — ✅ CLOSED Aug 12, 2026:** `.github/workflows/eslint.yml` deleted; verified `ci.yml` step "Lint" (`npm run lint`, line 18-19) is the real ESLint 9 + flat-config gate. `rg -l "\.eslintrc"` across the repo (excluding `docs/`) → 0 hits; workflow directory is now 6 files (ci, osv-scanner, semgrep, sonarcloud, sonarqube, trivy). SARIF upload not added — decision recorded: no Security-tab ESLint alerts requested; `semgrep.yml` + `osv-scanner.yml` already feed the Security tab. Docs closed: EXECUTION_SPEC §29, DOCS_NEEDS_CODE, audit §3, CONTEXT/README workflow counts (8 → 6).

**CI-02 — `trivy.yml` is unedited template boilerplate and fails before Trivy runs [Medium].** File: `.github/workflows/trivy.yml:20-23` — `docker build -t docker.io/my-organization/my-app:${{ github.sha }}` (literal placeholder) and no `Dockerfile` exists in this repo (Vercel-style deployment). Requirement: delete the workflow, or repoint it at `trivy fs .` (filesystem/dependency + IaC misconfiguration + secret scanning) which needs no Dockerfile and complements `osv-scanner.yml`. Acceptance: one decision recorded in this ticket; the trivy job either no longer exists or passes a clean run.

**Status — ✅ CLOSED Aug 12, 2026 (decision: delete).** Rationale recorded per ticket: (1) the job can never succeed — `docker build` against a nonexistent `Dockerfile`; (2) repointing to `trivy fs .` would duplicate `osv-scanner.yml`'s dependency-CVE coverage while its remaining value (secret scanning, IaC misconfig) is already provided by the kept configured platform — `semgrep.yml`'s managed rules (plus GitHub's native secret scanning); (3) there is no IaC to scan (no Docker/K8s/Terraform files anywhere); (4) consistent with the audit's own §8 principle "run 3 real checks, not 7 decorative ones" and with CI-03's consolidation trajectory (single static-analysis platform = Semgrep). `.github/workflows/trivy.yml` deleted; workflow directory now 5 files (ci, osv-scanner, semgrep, sonarcloud, sonarqube). Docs closed: EXECUTION_SPEC §29, DOCS_NEEDS_CODE, audit §3, CONTEXT/README workflow counts (6 → 5).

**CI-03 — Consolidate the three static-analysis integrations to one configured platform [Low-Medium].** Files: `.github/workflows/sonarcloud.yml:57-58`, `.github/workflows/sonarqube.yml:38` (both have blank `sonar.projectKey`/`sonar.organization`/`sonar.host.url` → fail or no-op on every push; running both SonarCloud and SonarQube is redundant), plus `semgrep.yml` (real rules configured; needs `SEMGREP_APP_TOKEN`/`SEMGREP_DEPLOYMENT_ID`). Requirement: keep one — Semgrep's free OSS tier is the lowest-friction and already configured — delete the two Sonar workflows. Acceptance: exactly one static-analysis job configured and green; two workflow files deleted.

**Status — ✅ CLOSED Aug 12, 2026:** `sonarcloud.yml` + `sonarqube.yml` deleted (both confirmed with blank `-Dsonar.projectKey=` / `-Dsonar.organization=`), keeping `semgrep.yml` as the single static-analysis platform per the ticket's requirement. Exactly one static-analysis workflow remains; workflow directory now 3 files (ci, osv-scanner, semgrep). **Follow-up (CI-03-adjacent, needs human):** confirm `SEMGREP_APP_TOKEN` + `SEMGREP_DEPLOYMENT_ID` secrets are set in the GitHub repo settings — if absent, `semgrep.yml` will fail on every push and needs a decision (set secrets vs. switch the action to `publishToken: ""` registry mode).

**CI-04 — Add `permissions:` block to `ci.yml`; pin first-party Actions to commit SHAs [Medium].** Files: `.github/workflows/ci.yml` (no `permissions:` key — the only workflow in the repo without one, so it inherits the repo default token scope), all `actions/checkout@v4`/`actions/setup-node@v4` references (mutable tags; third-party actions are already SHA-pinned). Requirement: add `permissions: contents: read` at the top of `ci.yml`, elevate per-job only where needed; pin `checkout`/`setup-node` to full commit SHAs; consider Dependabot `github-actions` ecosystem updates to keep pins current. Acceptance: `ci.yml` declares `permissions`; no `@v4`-style tag reference remains in any workflow.

**Status — ✅ CLOSED Aug 12, 2026:** `ci.yml` now declares `permissions: contents: read` (no elevation needed — the lint/typecheck/test/build steps only read contents; the bundle-budget step needs nothing more); `actions/checkout` pinned to `11d5960a326750d5838078e36cf38b85af677262`, `actions/setup-node` to `49933ea5288caeca8642d1e84afbd3f7d6820020`, and `github/codeql-action/upload-sarif` (semgrep job) to `c3400c2f38909e0dcf3c3a41f2030a8217be5d3e` (SHAs resolved from the official tags via the GitHub API at close-out time); `osv-scanner.yml` was already SHA-pinned with a `permissions` block. `rg "@v[0-9]" .github/workflows` → 0 hits. Dependabot `github-actions` updates not added — recorded decision: solo repo, pins refreshed manually at upgrade time; revisit if the workflow set grows.

**Status — ✅ CLOSED Aug 16, 2026 (commit `d8edcc6`).** `public/icon-192-maskable.png` + `icon-512-maskable.png` generated from the vector `icon.svg` (render → crop the colored circles → scale to 76% of canvas, centered on the #0F0A00 background): bbox verified programmatically against the W3C 80%-diameter safe zone (512: x[61,449]; 192: x[23,167] — within [52,460]/[20,172] respectively; reproducible via `/home/ubuntu/verify_safezone.py`). Existing full-bleed `icon-192.png`/`icon-512.png` remain `purpose: any` only. Manifest now contains `id`, `scope`, `categories: ["productivity"]`, `screenshots` (one `form_factor: wide` 1280x800 + one `narrow` 390x844, real captures of the sign-in page in `public/screenshots/`), and a `shortcuts` entry: Quick Capture → `/?capture=1` (icon: `icon-96.png`, new any-purpose 96px). The shortcut actually works because the app home page now mounts `src/components/layout/CaptureShortcut.tsx` (client): when `capture=1` is present it opens the existing client capture modal once per mount, with no URL rewrite. Build green; tests 181/181. Chromium install-dialog rich variant is the install-time effect of the screenshots field (verify on a real device — no headless automation exists for the dialog itself).

**PWA2-01 — Dedicated maskable icon + manifest completeness [Medium].** File: `public/manifest.json:9-13` — the same `icon-192.png`/`icon-512.png` are declared with both `"purpose": "any"` and `"purpose": "maskable"`; full-bleed art gets cropped by Android adaptive-icon launchers (subject must sit in the center ~80% safe zone). Manifest also missing `screenshots`, `shortcuts`, `id`, `categories`, `scope`. Requirement: generate safe-zone-padded maskable icons at 192/512, keep existing icons as `any`-only, add the missing fields (`scope: "/"`, `screenshots` with one wide + one narrow `form_factor`, a `shortcuts` entry for Quick Capture). Acceptance: `maskable.app` verification passes; manifest contains the new fields; install dialog shows the rich (screenshot) variant in Chromium.

**INFRA-23 — Edge Functions: drop deprecated import sources; fix cron check-then-insert race [Low-Medium].** Files: `supabase/functions/cron_cleanup/index.ts:1-2`, `supabase/functions/cron_recurrence/index.ts:1-2, 96-116` — both import `serve` from `https://deno.land/std@0.192.0/http/server.ts` (deno.land/std is maintenance-mode, deprecated in favor of JSR) and the Supabase client from `esm.sh` (known resolution issues for `@supabase/supabase-js`; `npm:` specifiers now the reliable pattern); `cron_recurrence` does a check-then-insert (`maybeSingle()` then insert) with no unique constraint or advisory lock, so overlapping invocations (retry, manual trigger) can duplicate a recurring task. Requirement: use built-in `Deno.serve` and `npm:@supabase/supabase-js@2`; add a partial unique index on `(user_id, title, recurrence) WHERE status = 'active'` in a new migration and treat insert conflict as success (fail-and-swallow) instead of the preceding `select`. Acceptance: both functions run under `Deno.serve`; a concurrent-invocation test yields exactly one recurring task.

**SEC2-02 — Verify Supabase Dashboard security settings against intent before launch [Medium, verify-before-launch].** File: `supabase/config.toml` (local CLI config; does not necessarily mirror the hosted project's Dashboard settings). Items to verify/decide: (a) `minimum_password_length = 6`, `password_requirements = ""` — 2026 guidance favors length (8–12 floor) over complexity rules, plus HaveIBeenPwned leaked-password checking; (b) `[auth.captcha]` commented out — no bot protection on public signup (hCaptcha/Turnstile supported); (c) `enable_confirmations = false` — email ownership never verified; may be an intentional low-friction choice, but must be stated explicitly. Also fold in the audit §4 verify-before-launch rows: Supavisor/transaction-mode pooling for serverless, PITR/backup RPO, and Edge Function invocation auth (both functions default to `verify_jwt = true` — confirm whatever triggers them sends a valid Authorization header, or cleanup/recurrence silently 401 forever; tie the "did the cron actually run" smoke check into OBS-01). Action: a 15-minute pass through the Dashboard's Security Advisor + Auth → Settings; record a decision for every item in this ticket. Acceptance: each item above has an explicit, dated decision recorded here.

**Status — ✅ CLOSED Aug 16, 2026 (commits `9651057` config, `a8dd007` functions, `63e3577` docs). Ticket candidate created for the residual client-side captcha gap: `SEC2-03` — Wire the Turnstile sitekey into the login form so legitimate signups keep working once the backend captcha enforcement is switched on.**

**`SEC2-03` status — ✅ CLOSED Aug 16, 2026 (commit `2ca3f48`).** Implementation: `src/lib/env.ts` adds an **optional** `NEXT_PUBLIC_TURNSTILE_SITEKEY` (non-throwing `.catch()` pattern preserved — invariant #1); new zero-dependency `src/components/features/TurnstileWidget.tsx` loads the Cloudflare Turnstile API lazily and exposes the challenge token via callback, with `error-callback`/`expired-callback` clearing the token so a stale token is never submitted; `(auth)/login/page.tsx` mounts the widget only when a sitekey is configured and disables the submit button until a token is issued; both server actions forward `cf-turnstile-response` to `signInWithOtp` via `captchaToken`. **Key safety property: until `NEXT_PUBLIC_TURNSTILE_SITEKEY` is set in the hosting environment, nothing renders and nothing changes — the backend captcha toggle can be flipped the moment this deploy is live AND the sitekey env var is set, in the same deploy window. Google OAuth requires no captcha token (OAuth flows bypass GoTrue's captcha check). Build green, tests 181/181, lint + type-check clean.**
- **(a) Password floor — DECIDED.** `minimum_password_length = 6 → 8` applied in `supabase/config.toml` (2026-08-16); `password_requirements = ""` kept deliberately — 2026 guidance (Supabase's own password-security docs) favors length over complexity, and the product is a low-friction personal tool. **HIBP — DECIDED ON, gated by plan.** `check_hibp = true` added to the local config mirroring the hosted "Prevent use of leaked passwords" toggle; the hosted Dashboard Auth → Settings toggle is the authoritative control (Pro plan and above per Supabase docs). Launch action: flip the Dashboard toggle; verify with a known-leaked password (e.g. `password12345`) that signup is rejected. Follow-up candidate: re-check this toggle after every plan change, since it can silently downgrade.
- **(b) CAPTCHA — DECIDED ON.** `[auth.captcha]` enabled with `provider = "turnstile"`, `secret = "env(SUPABASE_AUTH_CAPTCHA_TURNSTILE_SECRET)"` (no secret committed; set via `supabase secrets set`). Backend enforcement starts the moment the Turnstile secret is set on the hosted project; GoTrue issues a `400 {"error_code":"captcha_failed"}` until the challenge token passes. **Residual follow-up (ticket candidate SEC2-03): the client login form does not yet render the Turnstile sitekey widget** — without it, legitimate signups will fail with `captcha_failed` once the backend enforces; enable the backend toggle only when the client sitekey wiring is ready, in the same deploy. Sitekey comes from the same Turnstile dashboard as the secret.
- **(c) Email confirmations — DECIDED OFF, explicitly.** `enable_confirmations = false` kept. Rationale: `[auth.email.smtp]` is still commented — no production SMTP is configured, so enabling confirmations would send no email and permanently lock out every new signup; `secure_password_change = false` likewise has no nonce transport. Product posture: low-friction consumer sign-up. **Launch prerequisite recorded:** SMTP (`[auth.email.smtp]` + hosting SMTP provider credentials) must be configured AND tested (signup confirmation + password reset actually delivered) BEFORE either `enable_confirmations` or `secure_password_change` is flipped on. Revisit after SMTP lands; this is a one-click change at that point.
- **Edge Function invocation auth — DECIDED + code change.** Neither repo file nor `config.toml` records how `cron_cleanup`/`cron_recurrence` are triggered (no pg_cron/`net.http_post` in `src/` or `supabase/`), so the invocation contract lives in the Supabase Dashboard's scheduled-function config — **launch action: confirm both scheduled functions are configured to send an Authorization header with a JWT (anon key is sufficient; the function holds its own service-role key internally).** As a defense-in-depth measure, both functions now return an explicit `401 {"error":"No Authorization header — scheduled invocation must send a JWT"}` when invoked without a header (instead of GoTrue's unauthenticated default path), making a broken trigger loud rather than silent. **Smoke check (tied into OBS-01's now-closed Sentry adoption):** add a daily "did the cron run" check — Sentry Function Traces/monitoring on the two scheduled invocations; a day with zero invocations or a 401 spike is the failure signal. Until a monitor is wired (follow-up candidate), the one-time launch verification is: invoke both functions manually from the Dashboard with the auth header and confirm 200 + expected side effects (or zero soft-deleted rows for a fresh table), then inspect the scheduled-function logs for a successful run within 24 h.
- **Supavisor / transaction-mode pooling — DECIDED AS LAUNCH ACTION (not code).** Next.js API routes on Vercel are exactly the serverless workload Supavisor transaction-mode exists for. Launch action: in the Dashboard's Database → Settings → Connection Pooling, verify the pooler is on with transaction mode and the app's `DATABASE_URL` uses the 6543 pooler port; confirm `pg_stat_activity` shows pooled (not per-Vercel-instance) connections under load.
- **PITR / backup RPO — DECIDED AS LAUNCH ACTION + related ticket.** PITR is plan-gated (Pro). Launch action: confirm PITR (or at minimum daily backups with the desired RPO) is enabled on the hosted project. Backup retention beyond the platform default is separately tracked by `TOOL-17` — not re-addressed here. Also verify RLS against the Security Advisor (audit §4 already confirmed RLS correct and service-role key server-only; SEC2-01's rate-limit fix closed the API-side row).

**PERF-13 — Stop stripping `console.error` in production builds [Low].** File: `next.config.ts:31` — `removeConsole: process.env.NODE_ENV === "production"` (boolean form strips every `console.*` call from client bundles, including `console.error`). Requirement: change to `removeConsole: { exclude: ["error"] }` so at least local debugging via a user's browser console remains possible; becomes largely moot once OBS-01 lands (errors go to the tracker). Acceptance: a production build retains `console.error` in client bundles (`rg` in `.next`).

---

## 30. Addendum 17 — Vercel React best-practices audit (Aug 13, 2026)

Source: full audit of `src/` against the Vercel React Best Practices guide (70 rules, 8 categories: waterfalls, bundle size, server performance, client fetching, re-render, rendering, JS perf, advanced patterns). Run Aug 13, 2026 by parallel file-by-file review; every `file:line` below spot-verified against the build at HEAD (`e3abd54`). No code was changed by the audit. Every finding was cross-referenced against this document's existing tickets first — sites already covered by `PERF-01`/`PERF-07`/`PERF-09`/`PERF-10a`/`PERF-10b`/`PERF-11`/`PERF-12`/`PERF-13` or `BUG-09`/`BUG-26` are NOT re-ticketed. Net result: no CRITICAL-level new findings; the previously closed high-impact rules stay closed. The new tickets below are the residue, ordered by the impact the guide assigns to their category, with the re-render cluster (the app's largest remaining client-side cost, per §28's ~4.4 s hydration/main-thread attribution) first.

### New tickets

**Status — ✅ CLOSED Aug 16, 2026 (commit `16ef791`). All 15 argument-less `useAppStore()` call sites in `src/` converted to per-field selectors: `useShallow((s) => ({ ... }))` for 11 multi-field sites (home dashboard, PomodoroTimer, CaptureModal, RitualOverlay, SearchModal, SettingsModal, ExploreDrawer, AppContentWrapper, AppInitializer, MobileDrawer, MobileTopBar), direct `(s) => s.userSettings` selectors for 4 single-field sites (people page, people/[id], AddPersonPanel, TaskAddPanel). Setter reads use `useShallow` because store setters are inline arrow functions recreated on every `set()` call (identity-unstable by design). `grep -rn "useAppStore()" src` returns 0 hits. Build green (VERCEL=1), tests 181/181, lint + type-check clean. (The remaining `useAppStore.getState()` calls are intentional static reads, not subscriptions.)

**PERF-14 — Zustand whole-store subscriptions: one write re-renders entire surfaces [High].** Files: `src/app/(app)/page.tsx:105` (home dashboard destructures the whole store — task lists re-render on any store write: per-mutation `lastMutations`, `activeTimer`, `captureModalPrefill`), `src/components/features/PomodoroTimer.tsx:70` (whole-store in an always-mounted full-screen overlay), `TaskAddPanel.tsx:174`, `CaptureModal.tsx:86`, `RitualOverlay.tsx:257-263`, `ExploreDrawer.tsx:54`, `AddPersonPanel.tsx:67`, `LocationAddPanel.tsx`, `src/components/layout/MobileDrawer.tsx:32`, `MobileTopBar.tsx:11`, `AppContentWrapper.tsx:9`. Requirement: convert every site to per-field selector reads — the pattern already established at `Navigation.tsx:43`, `CalendarTaskChip.tsx:36`, `TaskCard.tsx:41` — or `useShallow` where a field group is genuinely needed. Acceptance: no argument-less `useAppStore()` call remains in `src/` (grep = 0 hit); the dashboard and Pomodoro overlay re-render only when a subscribed field changes (verify with React DevTools Profiler during a typing/stateful interaction). Priority: High — per `rerender-defer-reads`; biggest single re-render lever the audit found.

**Status — ✅ CLOSED Aug 16, 2026 (commit `[33ma049551[m`).** Single-pass bucketing implemented in `src/app/(app)/do/page.tsx`: the two filter passes + four bucket passes were replaced by one memoized derivation (`useMemo` over `[tasks, categoryFilter]`) — a start-date sweep, then a single loop classifying every remaining task into `overdue`/`today`/`upcoming`/`someday` with at most two `new Date()` parses per task (the second only for the same-day check, when the deadline is on or after `now`). Semantics verified against the old four-pass code with a synthetic equivalence harness covering all six category-filter branches plus start-date exclusion and <now / ≥now same-day / >now boundary cases — all MATCH, in original order. The four bucket arrays now hold referential identity whenever `tasks` and `categoryFilter` are unchanged, so the memoized `Column`/`TaskCard` tree from PERF-01 no longer defeats memoization on unrelated renders. Acceptance: one date parse per task per derivation (review); identity stability via `useMemo` deps. Build green (VERCEL=1), tests 181/181, tsc clean.

**PERF-15 — `/do` column bucketing: four-pass filter, per-task date reparse, fresh array identities every render [High].** File: `src/app/(app)/do/page.tsx:361-392` — four consecutive `filtered.filter(...)` passes (`overdue` 379, `today` 385, `upcoming` 390, `someday` 392, the `isActiveOrOverdue`/`toDateString()` helpers at 367-375); each pass constructs `new Date(t.deadline)` and calls `toDateString()` per task, ~4 date constructions + ~4 string allocations per task per render, no `useMemo`. Compounded: the four arrays are new identities every render, so the memoized columns/`TaskCard`'s `React.memo` (which `PERF-01`'s `useCallback` work exists to protect) are defeated on every parent state change. Requirement: one single-pass bucketing loop with a single `new Date(t.deadline)` per task (or a `useMemo`'d `Map` keyed by bucket) deriving all four arrays. Acceptance: one date parse per task per derivation (code review); the four arrays keep referential identity while the underlying `filtered` array is unchanged, so the memoized tree holds on unrelated state changes. Priority: High — the page behind §28's measured baseline (main-thread ~4.4 s), and the only ticket in this batch that plausibly moves that number client-side without a network change. Folds in both `js-combine-iterations` and `rerender-derived-state`.

**Status — ✅ CLOSED Aug 16, 2026 (commit `[33mb97d670[m`).** `MonthView.tsx` and `WeekView.tsx` now build a memoized `Map<yyyy-MM-dd, Task[]>` once per render (`useMemo` over `[tasks]` — one `parseISO` per task per derivation, O(1) per cell). `getTasksForDay` reads the map by day key in both views, so the ~42 MonthView cells and the WeekView all-day rows/timed columns never re-scan or re-parse. `calculateOverlap` in `WeekView.tsx` precomputes a numeric `start`/`duration` per task in a single pass; the sort comparator and the per-column placement loop read the precomputed fields (the inner loop previously re-parsed `lastInCol.deadline` twice per column check). Semantics verified with a synthetic harness on overlapping-event chains — old vs new layout output MATCH exactly (task identities, columns, widths). `parseISO` remains only inside the memoized index builders; no per-cell deadline parsing survives in either view. Build green (VERCEL=1), tests 181/181, tsc clean.

**PERF-16 — Calendar date math: per-cell/per-column rescans and comparator re-parsing [Medium].** Files: `src/components/features/calendar/MonthView.tsx:152-157` (`getTasksForDay` filters the full task list and runs `parseISO` per task for all ~42 grid cells per render, re-run on every `popoverDay` change), `WeekView.tsx:203-208, 271, 305, 337` (same helper called 2-3× per day per render, compounded by the parent's 60 s clock), `WeekView.tsx:42-98` (`calculateOverlap`'s sort comparator calls `parseISO(a.deadline)`/`parseISO(b.deadline)` per comparison, then re-parses `lastInCol.deadline` twice per column check). Requirement: one `useMemo`'d `Map<yyyy-MM-dd, Task[]>` per view — a single `parseISO` per task per derivation — read by `getTasksForDay` and the overlap math in O(1) per cell; parse each deadline to a numeric start once up front in `calculateOverlap`. Acceptance: per-render `parseISO` count drops from O(days×tasks) to O(tasks) (code review); no rescan when the underlying task array identity is unchanged. Priority: Medium — computation, not paint; complements (not duplicates) `TOOL-07`'s paint-side coverage of the same surfaces. Folds in `js-index-maps`/`js-set-map-lookups` and `js-cache-function-results`.

**Status — ✅ CLOSED Aug 16, 2026 (commit `[33m6e9737e[m`).** All five sites fixed: `think/[id]` thread + explores queries now run concurrently via `Promise.all` in `fetchThread` (still re-fired by the same two `useRealtime` hooks — behavior unchanged); `explore/[id]` starts the threads fetch immediately after the item lookup (a promise created right after the item call, awaited alongside `setLoading`) so threads no longer waits on the `revisited_at` mutation; `remember/people/[id]` runs person + linked-items via `Promise.all` — the items query now depends only on the route `id`, with the person row set when present and linked tasks always populated; both inbox `getUser()` round trips (remember + location branches) replaced by `item.user_id` already in hand from the inbox query (`InboxItem` carries `user_id`); `api/account` `DELETE` fails fast — the `SUPABASE_SERVICE_ROLE_KEY` check moved before the server-client creation, `getUser()` auth round trip, Redis rate-limit, body parse, and schema validation. Acceptance: zero `getUser()` calls that re-derive data already in hand (grep in `inbox/page.tsx` = 0 hits); queries issued concurrently (code review); account route fails fast on a missing service-role key. Build green (VERCEL=1), tests 181/181, tsc clean.

**PERF-17 — Detail-page fetch waterfalls; redundant auth round trips [Medium].** Files: `src/app/(app)/think/[id]/page.tsx:196-212` (thread query then explores query — independent, serialized, re-fired on every realtime event), `src/app/(app)/explore/[id]/page.tsx:68-111` (item fetch → `revisited_at` mutation → threads fetch; threads depends on nothing and is delayed behind a mutation), `src/app/(app)/remember/people/[id]/page.tsx:55-64` (items query nested inside the person query's success branch — false dependency; it needs only the route `id`), `src/app/(app)/inbox/page.tsx:286-289, 401-404` (a `getUser()` round trip where `item.user_id` is already in hand), `src/app/api/account/route.ts:42-50` (cheap env check evaluated after the auth/Redis/body-read work). Requirement: `Promise.all` the independent fetches in each detail page; make the people-`[id]` items query independent of the person row; drop the inbox `getUser` calls in favor of `item.user_id`; hoist the env check first in the account route. Acceptance: each detail page's fetch issues its queries concurrently (network tab while poking a realtime event); zero `getUser()` calls that re-derive data already in hand; the account route fails fast on a missing service-role key without touching auth/Redis. Priority: Medium (the inbox and account-route items are Low on their own; bundled deliberately to keep the session count down). Folds in `async-parallel`/`async-dependencies`/`async-cheap-condition-before-await`.

**Status — ✅ CLOSED Aug 16, 2026 (commit `[33m96fc084[m`).** The settings gate stays inline (render-critical gating per `PERF-12` attribution), and the items-count query stays inline because the stale/data-less redirect decision needs it before paint. Only the upsert moved: it now runs inside `after()` (imported from `next/server`) with a fresh `(await createClient())` — the earlier render-path client was not safe to reuse since `cookies()` is request-scoped and is drained after the response begins. The BUG-38 error-checking discipline is preserved (destructured `error` + console.error, plus a try/catch around the deferred client creation in case `cookies()` access throws post-response). Redirect to `/onboarding` for data-less stale users stays inline. Tests 181/181, build green (VERCEL=1), tsc clean. TTFB improvement lands on the `after()` post-response path — the ~300 ms class upsert round trip no longer extends the response for stale users with data.

**PERF-18 — Unblock first paint: onboarding auto-complete off the layout's critical path [Medium].** File: `src/app/(app)/layout.tsx:51-70` — the onboarding auto-complete (`items` count query at 51-54 then `user_settings.upsert` at 58-63) is awaited inline in the server layout, blocking first paint for every user in the stale-settings path; `after()` from `next/server` is unused repo-wide (grep = 0 hits, and `<Suspense>` likewise appears nowhere in `src/` except tests). The layout's settings round trip itself stays exactly as `PERF-12` attribution approved — it is render-critical gating — but the count+upsert is not and is separable. Requirement: wrap the count + upsert in `after()` (post-response, non-blocking). Acceptance: warm-curl TTFB on an authed route for a stale-settings user drops by roughly the measured count+upsert round trip (re-measure per §26.1 instrumentation; the ~300 ms class matches PERF-12 task-4's attribution of the settings call); onboarding auto-complete behavior unchanged (Playwright pass against a seeded stale user). Priority: Medium. Folds in `server-after-nonblocking`/`async-defer-await` and records the zero-`Suspense` observation rather than ticketing it separately.

**Status — ✅ CLOSED Aug 16, 2026 (commit `[33mc89ba35[m`).** `next/dynamic` with `ssr: false` is not permitted inside a Server Component (`layout.tsx` is a server layout), so the change follows the established `DynamicModals.tsx` escape hatch: new client wrapper `src/components/layout/RitualOverlayDynamic.tsx` (`"use client"` + `dynamic(() => import("@/components/features/RitualOverlay").then(m => ({ default: m.RitualOverlay })), { ssr: false, loading: () => null })`) renders the lazy overlay, and `layout.tsx` swaps its static import for `RitualOverlayDynamic`. Bundle verification against a true-HEAD baseline build: the RitualOverlay component + `react-textarea-autosize` (~36 KB) previously sat in the `(app)/layout` entry chunk list (a 136 KB ritual chunk loaded on every protected route); after the change, no layout-entry chunk contains `RitualOverlay` code — the component lands in a lazy-only chunk loaded on first triage open, while only the ~76 KB wrapper/loader chunk remains in the entry (Turbopack's dynamic machinery). Ritual UX paths unchanged (same component, `isOpen`/store behavior untouched); only the shared chunk list shifted. Tests 181/181, build green (VERCEL=1), tsc clean.

**PERF-19 — RitualOverlay lives in the app-shell bundle of every `(app)` route [Medium-High].** Files: `src/app/(app)/layout.tsx:8` (static import) and `:99` (rendered in the shared shell), `src/components/features/RitualOverlay.tsx:24` (statically pulls `react-textarea-autosize` into the shared chunk). The overlay is conditionally visible (ritual triage prompt) yet ships in every authed route's initial JS. Requirement: convert to `next/dynamic` with `ssr: false` — the exact pattern `PERF-12`/`DynamicModals.tsx` established — loaded on first triage open. Acceptance: `RitualOverlay` and `react-textarea-autosize` no longer appear in the shared-shell chunk (webpack analyzer, `ANALYZE=true next build --webpack`); ritual UX unchanged (step through a ritual from a cold page load). Priority: Medium-High — small mechanical change with a deterministic bundle cut across all six `(app)` routes, per `bundle-conditional`.

**Status — ✅ CLOSED Aug 16, 2026 (commit `[33m99c0202[m`).** Three preload sites wired: (1) `AppContentWrapper.tsx` ⌘K/Ctrl+K branch calls `SearchModal.preload()` before opening — the chunk begins fetching on keydown-anticipation; (2) `/do`'s "Add task" button fires `TaskAddPanel.preload()` on `onMouseEnter`/`onFocus`; (3) the SegmentedControl "Calendar" segment fires `CalendarView.preload()` on `onMouseEnter`/`onFocus` (the control's `options` type gained optional `onMouseEnter`/`onFocus` via a new `OptionProps<T>` interface, and each rendered segment button forwards them — an additive, non-breaking change). Per `bundle-preload`, no preload is attached where the first input *is* the trigger: `c`/`/` shortcuts, the ⌘K-triggered capture modal, SettingsModal/PomodoroTimer stay untouched. The four DynamicModals consts and the two `/do` dynamic consts are now `export const` so the same-module references can call `.preload()`; Next's `dynamic()` return type omits `preload`, so call sites use a minimal intersection cast (`as typeof X & { preload: () => void }`). `preload()` is a no-op once the chunk is loaded, so repeat calls are free. Acceptance (mechanical — the network-timeline check in the spec is a browser-observation step, so code evidence is recorded instead): `grep -rn "preload()" src` = 4 hits, each at the described trigger; hover/focus handlers are pointer/keyboard-device events only, so touch-first and keyboard-only first interactions never fire them. Tests 181/181, build green (VERCEL=1), tsc clean.

**PERF-20 — Preload the likely dynamic boundaries on hover/focus [Medium].** Files: `src/components/layout/DynamicModals.tsx:7-25`, `src/app/(app)/do/page.tsx:35-47` (TaskAddPanel, CalendarView), `src/components/layout/AppContentWrapper.tsx:28` (⌘K handler) — all `next/dynamic` boundaries use `loading: () => null` and none call `.preload()`; ⌘K thus pays the full chunk fetch/eval on the keypress itself. Requirement: call the dynamic modules' `.preload()` on their trigger's hover/focus/pre-keystroke as appropriate per discipline `bundle-preload` — SearchModal on ⌘K keydown-anticipation, TaskAddPanel/CalendarView on hover of their triggers — no preload when the first input *is* the trigger (no wasted transfer). Acceptance: network timeline shows each chunk fetched before its surface opens; preloads never fire for keyboard-only or touch-first first interactions. Priority: Medium; deliver together with PERF-19 (same `DynamicModals` region).

**PERF-21 — AppInitializer re-runs theme application and writes localStorage on every navigation [Low].** File: `src/components/layout/AppInitializer.tsx:63-83` — the theme-apply effect lists `pathname` in its deps, so every client-side route change re-runs `applyDocumentTheme` and up to 3 synchronous `localStorage.setItem` writes (`presense_theme`, `presense_color_mode`, `presense_density`) even when settings are unchanged. Requirement: key the effect on the settings values it actually computes from (or early-return when computed theme/mode/density are unchanged), dropping `pathname`; first-run behavior identical. Acceptance: navigating between `(app)` routes performs zero `localStorage.setItem` calls while settings are unchanged (temporarily instrument and count, then revert instrumentation); theme still applies on first paint via the inline boot script. Priority: Low. Per `js-cache-storage`; also removes a small per-navigation main-thread cost on the §28-troubled route.

### New tickets (production-risk review, Aug 20, 2026)

**SEC2-04 — CSP `connect-src` blocks Sentry ingest: all browser errors are invisible [Critical].** Files: `src/proxy.ts:32-55` (`buildCspHeader` — `connect-src` is `'self' https://*.supabase.co wss://*.supabase.co`; the `cspReportUri` helper at :23-27 already parses `ingestHost` from the DSN for the `report-uri`, but does not add the ingest host to `connect-src`). Sentry ingest runs on `*.ingest.de.sentry.io` (org is in Sentry's EU region) — confirmed live Aug 20, 2026: after `NEXT_PUBLIC_SENTRY_DSN` landed in Vercel and the redeploy went out, Sentry opened two issues within 25 minutes (PRESENSE-1: "Blocked 'connect' from 'o4511896684789760.ingest.de.sentry.io'"; PRESENSE-2: blocked `*.vercel.app` — a browser-extension/preview-alias artifact, ignore). Consequence: the browser SDK cannot POST envelopes, so zero client-side events reach Sentry despite correct DSN + report-uri (the report-uri is what exposed the gap). Requirement: extend `connect-src` with `https://${ingestHost}` when a DSN is configured, keeping the no-DSN CSP byte-identical to today; if Turnstile is enabled, add `https://challenges.cloudflare.com` too (deferred until `NEXT_PUBLIC_TURNSTILE_SITEKEY` is actually set — do not add preemptively). Acceptance: `middleware.test.ts` asserts `connect-src` contains the ingest host for EU (`ingest.de.sentry.io`) and US DSNs, byte-identical CSP when no DSN; live CSP header contains the ingest host after redeploy; no new Sentry CSP violation issues. Depends on: nothing. Conflicts: none — does not remove or weaken the CSP or nonce system.

**INFRA-24 — Supabase migration drift: INFRA-23 dedup index never applied to the hosted database [High].** Files: `supabase/migrations/20260817000003_items_unique_active_recurring.sql` (in repo, adds partial unique index `(user_id, title, recurrence) WHERE status = 'active'`) vs. the hosted DB (project `mhfzmgrrtruxuiscvbhm`, ap-southeast-2): `supabase_migrations.schema_migrations` last applied version is `20260817000001` — `000003` is missing while `000000`/`000001` (RLS cache/split) are applied. Without the index, `cron_recurrence`'s insert-conflict-as-success dedup (INFRA-23, closed Aug 16) is ineffective and overlapping invocations (retry, manual trigger) can still duplicate recurring tasks. Requirement: apply the pending migration to the hosted project (`supabase migration up`, or SQL via Dashboard). Acceptance: `20260817000003` appears in the hosted `supabase_migrations.schema_migrations`; a unique-constraint test insert (deliberate duplicate, then rollback) succeeds against the index. Depends on: nothing. Conflicts: none.

**SEC2-05 — Merge `audit/aug19-backend-fixes` and ship its four fixes [High].** Branch verified on GitHub: 4 commits (`60f5d81` AUDIT-01 SSR-safe avatar accent lookup table; `9cfe611` AUDIT-02 JSON 401 for unauthenticated `/api/*`; `7c38da4` AUDIT-04/05 cron `x-cron-secret` gating + per-table cleanup reporting; `bb617d1` AUDIT-06 purge 10 user tables on account deletion), build green, 198/198 tests. Live evidence the SSR crash is real and recurring: 7-day Vercel error cluster "ReferenceError: getComputedStyle" — 8 events, 2 users, `/` and `/trash`, last hit Aug 19 on build `dpl_8mDDNUJHb` (server-rendered initial pass of `Navigation`'s avatar accent expression inside server-component `(app)/layout.tsx`). Requirement: merge the branch to `main` and redeploy; re-verify the SSR crash cluster stops after the deploy. Acceptance: branch merged; production redeployed; no `getComputedStyle` cluster after the new build serves traffic; `GET /api/account` unauthenticated returns JSON 401 (not redirect). Depends on: nothing (branch self-contained). Conflicts: none — audit branch built on main, `npm run build` + tests green. **Status — ✅ CLOSED Aug 20, 2026.** Merged via PR #14 (merge commit, all four audit commits preserved: `60f5d81`, `9cfe611`, `7c38da4`, `bb617d1`); stale `agent-loop/2026-07-*` branches deleted. Live-verified on the merge-triggered production deployment (`dpl_8onBMF`): unauthenticated `GET /api/account` → `401 {"error":"Unauthorized","code":"AUTH_REQUIRED"}` (AUDIT-02); pages still 307 to `/login`; zero `getComputedStyle` SSR crash events and zero Redis/Sentry env errors on the new build (SEC2-04 + AUDIT-01 shipped together); the only remaining `[env]` log line is the optional `NEXT_PUBLIC_TURNSTILE_SITEKEY`. INFRA-25's cron-secret sequence now becomes the merge-blocking follow-up: set `CRON_SECRET` on the hosted Supabase project and add the `x-cron-secret` header to both scheduled triggers **before** the next cron window, otherwise recurrence/cleanup return 401 silently.

**TOOL-11 — Add `SENTRY_AUTH_TOKEN` for source maps and releases [Medium].** Evidence: latest build log (dpl_9MnyGZ, Aug 20) — "No auth token provided. Will not create release. Will not upload source maps." Every Sentry issue shows minified chunk names. Requirement: human creates a Sentry auth token (`project:write`) and adds it to Vercel as `SENTRY_AUTH_TOKEN` (Production only); after redeploy a Sentry release exists with uploaded sourcemaps. Acceptance: next build's Sentry step reports a created release; an issue's stack trace shows original file names and lines. Depends on: nothing (env-var-only). Conflicts: none.

### New tickets (secperf audit, Aug 20, 2026)

**SEC3-01 — Lock down RPC exposure: three trigger functions and `rename_category` are callable by `anon` [High].** Evidence (live `pg_proc` introspection): `handle_new_user()`, `increment_time_spent()`, `remove_linked_person()` are SECURITY DEFINER trigger functions with `anon_exec = true, auth_exec = true` — PostgREST exposes every `public` function as `/rest/v1/rpc/*`, so any unauthenticated caller can hit them. Full `prosrc` review found no exploitable path (bodies only reference `NEW`/`OLD` trigger tuples; direct invocation updates zero rows, and `handle_new_user`'s INSERT dies on `user_settings.user_id NOT NULL` for anon), but the exposure invites spam/log noise and sets a dangerous precedent. `rename_category` is the legitimate RPC and is properly guarded (`auth.uid()` check, length validation, key whitelist, no dynamic SQL) — it needs only `anon` revoked. Requirement: `REVOKE EXECUTE ON FUNCTION public.handle_new_user() FROM anon, authenticated;` same for `increment_time_spent()`, `remove_linked_person()`; `REVOKE EXECUTE ON FUNCTION public.rename_category(text, text, text, text) FROM anon;` — also record as migration `20260820000000` in the repo so all environments match. Acceptance: privilege matrix shows the three triggers `{anon:false, authenticated:false}` and `rename_category {anon:false, authenticated:true}`; `curl /rest/v1/rpc/handle_new_user` without auth returns 401/403; `rename_category` with a real JWT still 200. Depends on: nothing. Conflicts: none — trigger execution runs under the function owner and is unaffected by role grants.

**SEC3-02 — `CRON_SECRET` runtime secret still unset: cron functions run in JWT-passthrough mode [Medium-High].** INFRA-25 closed the launch sequence (jobs created, pg_net installed, secret-gated bodies deployed), but the gating only activates once the secret exists: Dashboard → Edge Functions → **Secrets** → Add secret `CRON_SECRET` = the value given Aug 20 during INFRA-25 (same one already configured in the `Authorization`/`x-cron-secret` headers of the three `cron.schedule` jobs — the exact value is in the INFRA-25 closure notes / `/home/ubuntu/infra25-state.md`; do not generate a new one, or the scheduled jobs' headers stop matching). Acceptance: `curl` either cron URL without `x-cron-secret` returns **401** (currently 200); next scheduled run returns 200 with `success:true`. Depends on: INFRA-25. Conflicts: none. Human action — no code change.

**PERF3-01 — Unindexed FK: `explores.linked_thread_id` [Medium].** Live `pg_constraint` + `pg_index` cross-check: `explores_linked_thread_id_fkey` (→ `threads.id`) has no covering index — cascade delete/update from `threads.id` seq-scans `explores`. Requirement: `CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_explores_linked_thread_id ON public.explores (linked_thread_id);` applied via `execute_sql` and recorded as a migration. Acceptance: index present in `pg_indexes`; `EXPLAIN SELECT id FROM explores WHERE linked_thread_id = '...'` shows Index Scan. Depends on: nothing. Conflicts: none (additive).

**PERF3-02 — Unindexed FK: `session_logs.task_id` [Medium].** Same class: `session_logs_task_id_fkey` (→ `items.id`) uncovered — cascade deletes and task-log lookups seq-scan. Requirement: `CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_session_logs_task_id ON public.session_logs (task_id);` via `execute_sql` + migration record. Acceptance: index present; `EXPLAIN` on `WHERE task_id = '...'` shows Index Scan. Depends on: nothing. Conflicts: none (additive). **Status — ✅ CLOSED Aug 20, 2026.** Applied with SEC3-01/SEC3-03/PERF3-01 as one migration `20260820000000_secperf_rpc_rls_fk_hardening.sql` (repo + recorded in hosted `supabase_migrations.schema_migrations`, now `20260817000000`/`000001`/`20260820000000`). Live-verified in `pg_indexes`: both `idx_explores_linked_thread_id` and `idx_session_logs_task_id` present. Lesson learned mid-flight: the callable-from-RPC grants come from the default `PUBLIC` grant (`=X/postgres` acl entry), so the first attempt (`REVOKE ... FROM anon, authenticated`) changed nothing — the fix is `REVOKE ... FROM PUBLIC` plus `GRANT EXECUTE ... TO authenticated` for `rename_category`. Verified via `pg_proc.proacl`: trigger trio now `{postgres,service_role}` only; `rename_category` `{postgres,authenticated,service_role}`. Policy consolidation live-verified: `push_subscriptions` and `user_settings` both report exactly 4 policies (was 5 each). One caveat: `CREATE INDEX CONCURRENTLY` cannot run inside the MCP connector's transaction block — each index was created as a separate single-statement call, which succeeded. **SEC3-01 — ✅ CLOSED Aug 20, 2026.** Same migration + `proacl` evidence as above. **SEC3-03 — ✅ CLOSED Aug 20, 2026.** Same migration; `pg_policies` count check = 4/4; the 8 other RLS tables untouched.

**SEC3-03 — Consolidate duplicate permissive policies on `push_subscriptions` and `user_settings` [Low].** Live `pg_policies`: both tables carry 5 policies — 4 correct split policies (`users_own_<table>_{select,insert,update,delete}`, roles `{authenticated}`, per-row-safe `(SELECT auth.uid() AS uid) = user_id`) plus one stale duplicate (`users_own_push_subs` / `users_own_settings`, cmd `ALL`, roles `{public}`, direct `auth.uid() = user_id`). The 8 other RLS tables follow the clean 4-policy pattern. Requirement: `DROP POLICY users_own_push_subs ON public.push_subscriptions; DROP POLICY users_own_settings ON public.user_settings;` — authorization is byte-identical before/after (the replacements cover every command); record in a migration with the exact reversal statements in comments. Acceptance: both tables report exactly 4 policies, all `{authenticated}`, all per-command; `EXPLAIN SELECT` on each table shows a simpler plan; anon still gets 0 rows everywhere. Depends on: nothing. Conflicts: none — no `auth.uid() = user_id` binding is removed anywhere.

### NOTED — secperf items deliberately not ticketed

- **`pg_trgm` in `public` (linter warning):** 10 live GIN indexes (`idx_items_title`, `idx_threads_title`, `idx_people_name`, `idx_explores_title`, `idx_explores_note`, `idx_locations_item`, `idx_locations_loc`, `idx_items_linked_people_ids`, `idx_threads_linked_people_ids`, `idx_items_first_step`) and all similarity/ILIKE queries resolve through `public.pg_trgm`. Moving the extension would break search entirely. Documented in `docs/plans/SECURITY_PERFORMANCE_AUDIT.md` (Aug 20, 2026 report). Not a ticket — acting on this linter finding would be destructive.
- **`pgrst_watch` mutable `search_path` (linter warning):** Supabase-managed `realtime` extension function; not user-modifiable. No action available.
- **Unused-index flags (linter):** `pg_stat_user_indexes` shows zero scans on most indexes, but the DB holds ≤9 live rows per table — the stats reflect genuine single-user usage, not waste. Re-audit when live rows exceed ~10k. No ticket.
- **Leaked password protection disabled (Auth warning):** dashboard-only toggle (Authentication → Settings → Email providers). Recorded as a 1-click human action, not a code ticket.

**INFRA-25 — Scheduled cron triggers must send `x-cron-secret` after SEC2-05 merges [Medium-High].** Files: Supabase Dashboard → Edge Functions → Schedules for `cron_recurrence` and `cron_cleanup` (verify_jwt=true default); branch version requires header `x-cron-secret` = `CRON_SECRET`. Launch sequence: (1) merge SEC2-05; (2) `supabase secrets set CRON_SECRET=$(openssl rand -hex 32)` on the hosted project; (3) add header `x-cron-secret` with that secret to both scheduled triggers before or immediately after the next cron window; (4) redeploy both functions if their hosted bodies predate the branch (`supabase functions deploy cron_recurrence cron_cleanup`); (5) verify both schedules return 200 in their next run (Dashboard logs). Until step 3, the crons return 401/403 silently — no recurrence, no cleanup. Acceptance: next scheduled run of each function returns 200 with expected side effects; no 401 spike. Depends on: SEC2-05. Conflicts: none. **Status — ✅ CLOSED Aug 20, 2026.** Diagnosed during launch: only ONE cron job existed on the hosted project (`cleanup_trash_daily`, id 1), the two recurrence/cleanup jobs had never been created there, **and `pg_net` was not installed** — every run since Aug 16 failed with `schema "net" does not exist` (verified in `cron.job_run_details` via `execute_sql`). Fixes applied directly to the hosted database (project `mhfzmgrrtruxuiscvbhm`): `CREATE EXTENSION pg_net` (extensions schema), and `cron.schedule` upserts created `cron_recurrence` (job 2, `5 * * * *`) and `cron_cleanup` (job 3, `0 1 * * *`), mirroring the `cleanup_trash_daily` `net.http_post` pattern with `Authorization` + `x-cron-secret` headers and 5 s timeout. Both functions deployed fresh (secret-gated bodies, AUDIT-04/05 code) and live-tested: WITH secret → 200 `success:true` (recurrence processed/created 0 — expected for idle day; cleanup all 5 tables `ok`); without secret → still 200 in passthrough because `CRON_SECRET` is not yet set on the project. The remaining step is human-only: Dashboard → Edge Functions → **Secrets** → Add secret `CRON_SECRET` = the value given Aug 20; gating activates with the next run, no deploy needed.

### NOTED (checked, deliberately not ticketed)

- **`TOOL-07` gap:** content-visibility wrappers exist for task/explore/thread rows (`globals.css:1356-1364`; `TaskCard.tsx:235`, `explore/page.tsx:95`, `think/page.tsx:340, 401`) but **inbox rows have none** (`inbox/page.tsx`) — recorded as an extension of `TOOL-07`'s next pass, not a new ticket ID.
- **Re-render residue:** `think/page.tsx:43-50, 320, 333` un-memoized `filteredThreads` (filter runs twice per render); `SettingsModal.tsx:879-917` rebuilds the `Intl.supportedValuesOf("timeZone")` options array (~400 entries) every render (`TIME_OPTIONS` is correctly hoisted — extend that pattern); `inbox/page.tsx` `InboxItemCard` is unmemoized with per-render `routeInboxItem`/`dismissInboxItem` handlers, so every row re-renders on any page state change. Also repo-wide: zero `useTransition`/`useDeferredValue` usage — revisit for the filter/search surfaces when `PERF-15`/`PERF-16` land.
- **LOW hygiene items (one-line fixes, batch when touching the file):** `Sheet.tsx:27-29` reads `window.visualViewport` inline per render although `useVisualViewport` exists; `LoadingSpinner.tsx:30, 58, 108` animates `m.svg` directly (animate a wrapper div per `rendering-animate-svg-wrapper`); `(app)/page.tsx:52` (RitualStatusBadge) renders `new Date()` in the render body with `suppressHydrationWarning` present only on `<html>` (`layout.tsx:55`) — add per-element suppression or gate the render; `Navigation.tsx:51-52` and `RitualOverlay.tsx:308-309` register duplicate per-instance `focus`/`visibilitychange` listeners; `capture-router.ts:168, 219-220` constructs `RegExp` per call (`js-hoist-regexp`).
- **`advanced-init-once` item needing a decision, do not change unilaterally:** `LenisProvider.tsx:21-51` initializes a new `Lenis` per route-mount with a perpetual `requestAnimationFrame` loop, and only `/think` + `/explore` get smooth scroll (mounted at `think/page.tsx:197`, `explore/page.tsx:258`) — moving it to the root layout is mechanical, but *why only two routes have it* is a design question this document has not answered.
- **`server-no-shared-module-state` flag-only:** `rate-limit.ts:5, 48` module-level Maps are the documented in-memory dev fallback (production fails closed), consistent with `SEC2-01`'s kept design — never "fix" without a decision.
- **Verified-clean rule families, for the record:** `async-api-routes` (GET handlers only where canonical); `server-auth-actions` (all in-scope routes check; magic-link rate limiting noted as a spam vector candidate, no ticket — login is the pre-auth surface); `server-cache-react`/`server-cache-lru` (N/A — Upstash is the cache layer); `server-hoist-static-io` (fonts module-hoisted, `layout.tsx:8-18`); `server-parallel-nested-fetching` (no layout+page nested server fetch exists); `bundle-barrel-imports` (zero `index.ts` barrels in `src/`); `bundle-analyzable-paths` (all 3 dynamic `import()` sites are static literals); `client-swr-dedup` (`staleTime` 5 min, `refetchOnWindowFocus: false` per `PERF-07`); `client-passive-event-listeners` (no scroll/wheel listeners at all); `rerender-no-inline-components`; `rendering-hoist-jsx`; `js-length-check-first`; `js-min-max-loop`; `js-tosorted-immutable`; `js-flatmap-filter`; `js-request-idle-callback` (Link prefetch covers it); `advanced-effect-event-deps`; `advanced-event-handler-refs` (`useRealtime.ts:36-40` is the exemplary pattern); `advanced-use-latest` (covered by the `useRef` latest-handler pattern).

## 31. Addendum 18 — sidebar polish pass (Aug 17, 2026)

### DS-15 — Sidebar polish pass (collapsed-state affordances)
- **Priority:** High (user-requested UI ticket, approved Aug 17, 2026)
- **Files:** `src/components/layout/Navigation.tsx` (sidebar only — `BottomNav` untouched), `src/app/globals.css` (sidebar tooltip styles only), `docs/plans/EXECUTION_SPEC.md` (this record)
- **Model decision:** Option A from `SIDEBAR-REDESIGN-PROPOSAL.md` — the hover/focus-expand model is preserved exactly (`w-[80px] hover:w-[248px] focus-within:w-[248px]`); **invariant 4 remains intact, no click-toggle is added.** `CONF-05` stays resolved as-shipped (pure hover-expand, no pinning).
- **Root cause of reported defects (per user's screenshots vs. code):** the collapsed rail carries only native `title` tooltips (slow, unstyled, invisible on touch); the Inbox row has no count affordance; the utility group (Focus/Trash/Settings) hangs at the bottom with a single faint 50%-opacity divider; and the ritual "Plan my day" row does not render in the collapsed state of the deployed build.
- **Requirement:**
  1. Collapsed rail: a styled hover/focus-revealed tooltip pill (`.sidebar-tooltip`) on every row — nav items, Quick Capture, ritual button, utility items, account — positioned to the right of the rail, dark surface, rounded, with the label text and the row's `Kbd` hint where one exists. Native `title` stays as the no-CSS fallback. Honors `prefers-reduced-motion` (instant reveal, no transition).
  2. Inbox count badge: a live unread count derived from the same `["inbox-tasks"]` query the inbox page uses (`items.status = 'inbox'`, user_id-filtered), subscribed via `useRealtime` refetch; collapsed state shows a small badge at the icon's top-right corner per M3, expanded state shows the numeral beside the label; count 0 = no badge.
  3. Icon consistency: all sidebar icons at size 20, strokeWidth 1.5 (utility rows currently render at 17/1.5 — align them to the 44px row grid used by the nav rows).
  4. Utility group: replace the 50%-opacity divider with a scannable full divider and consistent spacing, utility rows on the same 44px grid with the same focus/hover contract.
  5. Ritual row visibility: verify "Plan my day" renders in the collapsed state in both dev and deployed builds; if the deployed absence is a build artifact rather than CSS, confirm by reproducing locally first.
- **Acceptance criteria:**
  1. Invariant 4 unmodified: the aside keeps `w-[80px] hover:w-[248px] focus-within:w-[248px]` with `group/sidebar`; no chevron, button, or persisted state added.
  2. Every collapsed-row tooltip is visible within ~300ms on hover and on keyboard focus; `prefers-reduced-motion: reduce` disables the transition.
  3. Inbox badge matches the live store count in both rail states and clears to invisible at 0.
  4. Ritual row visible in collapsed state; utility rows on the 44px grid with a clear divider.
  5. `npm run build` (with `VERCEL=1`) passes; `npm test` passes; `BottomNav` behavior unchanged; `MobileDrawer`/`MobileTopBar` untouched.
...
**DS-16 status (Aug 17, 2026 — commit `f7c95d4`):** ACCEPTANCE CRITERIA MET — (1) collapsed rail now shows clear hierarchy: `.sidebar-brand-tile` rounded-square containers anchor the top (brand mark) and bottom (account avatar), Quick Capture is a softened solid-accent row (h-10, `shadow-[0_2px_12px_-2px_var(--accent)]`, no heavy glow), and all nav/tool rows are muted at `text-3` until hovered or active; (2) active row is unmistakable via `.nav-row-active` (accent-dim pill + `box-shadow: inset 3px 0 0 0 var(--accent)` left bar), applied to Home/Inbox/Do/Remember/Think/Explore matches, /trash, and the ritual row while pending; hover raises rows to surface-hover with text-1; (3) thin tool-row divider removed, replaced with uppercase "Tools" block label (expanded-state-only via `group-hover/sidebar:block`) and spacing rhythm; "Spaces" label precedes the nav block; (4) ritual row integrated as a regular nav row — pending = accent pill treatment, done states muted, no separate outline pill; (5) DS-15 inbox badge restyled as `.sidebar-badge-v2` offset inside the 40px icon tile's top-right corner with a 2px surface outline ring; (6) invariant 4 preserved exactly (`w-[80px] focus-within:w-[248px] hover:w-[248px]`, `group/sidebar`); (7) build (`VERCEL=1 npm run build`) passes, tests pass (16 files, 181), `BottomNav`/`MobileDrawer`/`MobileTopBar` untouched.

## 33. Addendum 20 — SettingsModal setState-in-render warning (Aug 17, 2026)

**Ticket:** BUG-45 — DynamicModals console warning: "Can't perform a React state update on a component that hasn't mounted yet... side-effect in the render function" at `DynamicModals.tsx:32` (<SettingsModal />), reported from `src/app/(app)/layout.tsx` shell. Next.js 16.2.9 (Turbopack).
**Type:** bug · **Priority:** P1 (console error visible to the user on every page load).
**Scope:** `src/components/features/SettingsModal.tsx` (primary), `src/components/layout/DynamicModals.tsx` (only if needed). SearchModal and PomodoroTimer already guard with `return null` when closed; CaptureModal's effects are individually gated.
**Requirement:**
1. SettingsModal is always mounted by DynamicModals (next/dynamic ssr:false) and has no closed-state guard — every hook runs on every render. The chain `const settings = watch()` (unselective, new object reference every render) → `useDebounce(settings, 1000)` → render-driven reference churn fires `reset(data)` and the theme/localStorage effect while a render is in flight, producing the warning.
2. Fix: SettingsModal renders `null` when `isSettingsModalOpen` is false (hooks must be reordered/moved into a guarded sub-component so order is stable), OR equivalently make the component a no-op inert render when closed. Replace unselective `watch()` with selective subscriptions for the save debounce and the theme effect (react-hook-form `useWatch` or field-name array form). Ensure `reset(data)` fires once per open cycle only.
3. Acceptance: no console warning on fresh load of any app-shell page; settings load/save/autoflush + theme switching behavior unchanged; build (`VERCEL=1 npm run build`) passes; `npm test` passes (16 files, 181 tests).
...
**BUG-44 status (Aug 17, 2026 — commit `22ceccd`):** RESOLVED — (1) hover/focus-revealed trash button added on People's `TodayPersonCard` (next to the Briefing Ready badge) and `SortablePersonRow` (after the grip handle), Explore's swipeable item (next to the type badge), Think's stale-thread and All-Threads cards (left of the pin button / top-right where no pin exists), Do's `TaskCard` (top-right; priority dot shifted to avoid overlap), and Locations rows (left of the trailing metadata column) — all `opacity-0 group-hover:opacity-100 transition-opacity focus-visible:opacity-100`, hidden on touch (`hidden md:flex`), 28px touch-friendly square, aria-labels "Move {name} to trash"; (2) every delete stops propagation/prevents navigation so row links and edit panels never fire; (3) Think and Locations gains new list-level soft-delete fns following DS-11 (optimistic remove + `moveItemToTrashPatch()` lifecycle patch — INFRA-19 — + "moved to trash" toast, error reverts to fetch); all rows share BUG-08's global trash pipeline; swipe affordances untouched; builds (`VERCEL=1`) and tests (16 files, 181) pass.

### 28.2 — DS-30 closed (Aug 17, 2026, commit 15d4cfd)
One hover system implemented: (1) `GlassCard` now scopes `overflow-hidden` to the genuinely-clipping hero variant only; hoverable cards use `hover:-translate-y-0.5` (2px, matching `TaskCard`'s `whileHover {{ y: -2 }}`) with duration 200ms and ease `[0.25,0.46,0.45,0.94]` plus a hover-shadow step — no more scale-based card hover anywhere (`hover:scale-*` removed from all six sites: explore, home task rows, locations, people ×2, think, do archived rows). (2) The inbox card's `!overflow-visible` override removed — fix is in GlassCard/source class now; inbox row instead carries the same lift+shadow+amber border emphasis. (3) `hover:-translate-y-px` variants also normalized to `-translate-y-0.5` for consistent magnitude. Scale transforms retained only for press feedback (`whileTap`), mount/pop animations, and small icon affordances — DS-30's ban applies to card hover. Build green, 185/185 tests. DS-30: **CLOSED**.

### 28.3 — DS-28 closed (Aug 17, 2026, commit 7ab41c1)
Space colors rebuilt as theme-derived OKLCH sets instead of four hand-picked hexes: `do` = the theme's own accent (hue/lightness/chroma copied from `--accent`), while `think` (H≈25 red-family), `remember` (H≈55 peach), and `explore` (H≈40 deep amber) hold **fixed semantic hues** with lightness from the warm reference set and chroma capped to the accent's chroma — so the four-way role *relationship* is identical in every theme and every mode, and each set still reads as belonging to its theme (navy's `do` becomes periwinkle, forest's becomes pale gold). Navy dark/light and forest dark/light gained explicit tokens (they previously inherited warm's); the warm light-mode block got the light-accent derivation (`#bf8c00`/`#c9403d`/`#e88a4a`/`#9e4424`), and each role's `rgba` dim/border variants were regenerated from the new hexes. Values computed with culori's OKLab and round-trip-verified; the warm-dark set stays nearly identical to the previous hand-picked palette (perceptual dE < 8 per role) so the existing visual identity is preserved. DS-02 (dead shadcn OKLCH tokens) left untouched — the `@theme` reset block is the active Tailwind v4 mapping for the still-imported shadcn layer, not dead tokens. Build green, 185/185 tests. DS-28: **CLOSED**.

### 28.4 — DS-29 closed (Aug 17, 2026, commit 60241df)
Glassmorphism 2.0 applied on top of the existing elevation-bundle system: (1) the grain/noise layer was already present on every glass surface via a single shared `::before` (SVG `feTurbulence`, opacity 0.05, `mix-blend-mode: overlay`) — kept as-is, still one static asset referenced by all surfaces, no per-frame computation; (2) new `--elev-*-*-frost` tokens add the 2026 alpha-gradient frost fill (top-brighter two-stop gradient) as a shared `::after` layer on every glass surface — `.glass-card`/`.glass-card-elevated`/`.glass-panel` use the raised tier, `.dropdown-panel`/`.mobile-top-bar`/`.bottom-nav`/toasts the floating tier, `.modal`/`.mobile-drawer` the overlay tier — one definition per tier, never duplicated per component; (3) a new `--elev-*-*-specular` token set adds the 0.5px top-edge highlight merged into each surface's own `box-shadow` declaration (modal/toast/in-place; others via later-cascade overrides). Dropdown and toast surfaces now carry the same material family as cards/modals — blur tier, grain, frost fill, and specular edge — at the `floating` tier; PERF-04/08's blur budget is unchanged (no new `backdrop-filter` usage added, existing tier assignments kept). Pills remain deliberately outside the glass language per DS-29 point 5 (vivid, saturated, glowing). Build green, 185/185 tests. DS-29: **CLOSED**.
