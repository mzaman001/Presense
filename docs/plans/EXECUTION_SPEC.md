...
**Status — ✅ Decisions recorded Aug 16, 2026 (commits `config:` + `functions:` — SHAs filled when pushed). Ticket candidate created for the residual client-side captcha gap: `SEC2-03` — Wire the Turnstile sitekey into the login form so legitimate signups keep working once the backend captcha enforcement is switched on.**
