...
**INFRA-25 — Scheduled cron triggers must send `x-cron-secret` after SEC2-05 merges [Medium-High].** Files: Supabase Dashboard → Edge Functions → Schedules for `cron_recurrence` and `cron_cleanup` (verify_jwt=true default); branch version requires header `x-cron-secret` = `CRON_SECRET`. Launch sequence: (1) merge SEC2-05; (2) `supabase secrets set CRON_SECRET=$(openssl rand -hex 32)` on the hosted project; (3) add header `x-cron-secret` with that secret to both scheduled triggers before or immediately after the next cron window; (4) redeploy both functions if their hosted bodies predate the branch (`supabase functions deploy cron_recurrence cron_cleanup`); (5) verify both schedules return 200 in their next run (Dashboard logs). Until step 3, the crons return 401/403 silently — no recurrence, no cleanup. Acceptance: next scheduled run of each function returns 200 with expected side effects; no 401 spike. Depends on: SEC2-05. Conflicts: none.
