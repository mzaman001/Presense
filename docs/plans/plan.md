...
**Bug:** The routing dropdown is clipped because it sits inside a scrollable/`overflow: hidden` container.

**Fix:** (Note: This was originally diagnosed as a z-index issue, but z-index cannot fix `overflow: hidden` clipping. See `BUG-27` in `EXECUTION_SPEC.md`.) The correct fix is to migrate this dropdown to use `Popover.tsx` (which uses React Portal to render to `document.body`) rather than relying on inline absolute positioning.

**Verify:** Open the inbox, scroll down so a card is near the bottom, click "Route it". The dropdown should appear unclipped.
