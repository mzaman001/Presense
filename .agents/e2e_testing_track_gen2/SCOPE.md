# Scope: Playwright E2E Realtime Subscription Testing Track (Gen 2)

## Architecture
- **Test Runner**: Playwright (`@playwright/test` already configured in package.json and playwright.config.ts)
...
|---|---|---|---|---|
| 1 | Test Infra Setup | Install `@playwright/test` and initialize Playwright configuration | None | DONE |
| 2 | Test Route & Middleware Setup | Create `/test-realtime` route and modify `src/middleware.ts` to allow bypass | None | DONE |
