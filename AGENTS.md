...
3. **Mobile viewport bugs (ROOT PATTERN 3)** — **CLOSED Aug 10, 2026 as stale**: all 7 `h-screen` instances migrated to `h-dvh` in `8c249b6` (July 7, pre-audit; `rg 'h-screen|100vh' src` = 0 hits); `Sheet.tsx` drag-handle fix landed in `ad79e81` (BUG-36/39); `Input.tsx` 16px iOS auto-zoom floor at `globals.css:1366-1375` (BUG-41). Verified by the Aug 10 sweep — see `EXECUTION_SPEC.md` §24.7. Do not re-fix.
4. **Design system fragmentation (ROOT PATTERN 4)** — 136 hardcoded hex break theming (was 99 at audit); 6 different hover magnitudes (DS-30 says translateY only); 44 raw `<input>` elements (some intentional per COMPONENT_MANIFEST). The native `<select>`/`type="time"`/`<datalist>` items (BUG-43/25/33) are **CLOSED** — verified gone Aug 10, 2026.
5. **Settings + schema bloat (ROOT PATTERN 5)** — 9 unused notification booleans for a non-existent push system; 4 redundant time fields (**UI collapse to 2 Dropdown time-pickers DONE — `quiet_start`/`quiet_end` columns still awaiting schema cleanup, CONF-14**); `ritual_streak` column actively written despite CONF-17 no-gamification resolution; 4 dead tables.
