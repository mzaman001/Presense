...
export async function DELETE(request: Request) {
  try {
    ...
    // No separate purge. Every public table with a user_id (items, threads,
    // locations, push_subscriptions, user_settings, categories, session_logs,
    // ritual_logs) references auth.users ON DELETE CASCADE, so deleteUser
    // removes the user's rows in the same transaction. The per-table sweep
    // that used to follow (AUDIT-06) always found nothing: eight extra
    // queries and a "partial purge" path that could not happen. Verified
    // against the live schema on 2026-09-24. A new user-owned table must
    // keep that cascade.
