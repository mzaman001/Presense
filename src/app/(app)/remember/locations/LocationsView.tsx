...
function daysAgo(dt: string | null, now: number): number {
  if (!dt) return 0;
  return Math.floor((now - new Date(dt).getTime()) / 86400000);
}
...
export function LocationsView({
  ...
  /** The unfiltered list, streamed by the server page; undefined if that failed. */
  itemsPromise: Promise<LocationItem[] | undefined>;
  /** The server's clock, so staleness badges match on hydration. */
  renderedAt: number;
}) {
  ...
  // A cached list renders at once; only a cold, unfiltered load reads the
  // server's stream. Searches always query from the client.
  const serverItems =
    !search.trim() && !queryClient.getQueryData(locationsQueryKey(""))
      ? use(itemsPromise)
      : undefined;
  ...
  const {
    ...
    queryKey: locationsQueryKey(search),
    queryFn: () => fetchLocations(supabase, userId, search),
      ...
      <div className="flex flex-col gap-6">
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {items.map((item, i) => {
              const days = daysAgo(item.updated_at, renderedAt);
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1 pr-20">
                        <p
                          // Element Timing: when item names first paint
                          // (React's types lack the attribute).
                          {...{ elementtiming: "location-name" }}
                          className={cn(
